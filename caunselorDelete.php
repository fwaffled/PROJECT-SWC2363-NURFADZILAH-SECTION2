<html>
    ...
    if ($_SERVER['REQUEST_METHOD']== 'POST')
    {
        ...
        {
            //make the query
            $q = "DELETE FROM caunselor WHERE caunselorID = $id LIMIT 1";
            $result = @mysqli_query($connect, $q);//run the query

            if (mysqli_affected_rows($connect) == 1)
            {
                echo '<script>alert("The user has been deleted");
                window.location.href="caunselorList.php";</script>';
            }
            else
            {
        ...
        {
            echo'<script>alert("The caunselor has NOT been deleted");
            window.location.href="caunselorList.php;</script>';
        }
    }
    else
    {
        ...
        $q = "SELECT caunselorName FROM caunselor WHERE caunselorID = $id";
        $result = @mysqli_query ($connect, $q);//run the query

        if (mysqli_num_rows($result) == 1)
        {
            //get caunselor information
            $row = mysqli_fetch_array($result, MYSQLI_NUM);
            echo '<form action="caunselorDelete.php" method="post">
